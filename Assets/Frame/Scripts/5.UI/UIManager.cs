...
using UnityEngine.UI;
    ...
    public class UIManager : ManagerBase<UIManager>
    {
        ...
        public T Show<T>(int layer = -1) where T : UI_PanelBase
        {
        ...
        /// <param name="layer">层级 -1等于不设置</param>
        public UI_PanelBase Show(Type type, int layer = -1)
        {
            if (UIElementDic.ContainsKey(type))
            {
                ...
                {
                    UI_PanelBase window = ResManager.InstantiateForPrefab(info.prefab, UILayers[layerNum].root).GetComponent<UI_PanelBase>();
